...
class GlossaryDatabase:
    ...
    def export_to_latex(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for category_name, comment in cursor.fetchall():
                # Prima sezione con nome categoria
                content += "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n"
                ...
                # Ottieni le entries per questa categoria
                cursor.execute('''
                    SELECT e.key, e.type, e.name, e.first, e.text, e.description
                    FROM entries e
                    JOIN categories c ON e.category_id = c.id
                    WHERE c.name = ?
                    ORDER BY e.key
    ...
    def import_from_latex(self, content):
        ...
        Args:
            content (str): Il contenuto del file LaTeX
        """
            ...
            for i, section in enumerate(sections):
                ...
                for line in lines:
                    if "DEFINIZIONI" in line:
                        ...
                        # Salva o aggiorna la categoria con il commento
                        ...
                        if exists:
                            ...
                            print(f"Aggiornato commento per categoria esistente: {current_category}")
                        else:
                            cursor.execute('''
                                INSERT INTO categories (name, comment) 
                                VALUES (?, ?)
                            ''', (current_category, category_comment))
                            print(f"Inserita nuova categoria: {current_category}")
                        
                        category_comment = None  # Reset per la prossima categoria
                        break
                
                # Resto del codice per le entries...
                if not lines[0].strip().startswith('% DEFINIZIONI'):  # Processa solo le sezioni con definizioni
                    cursor.execute('SELECT id FROM categories WHERE name = ?', (current_category,))
